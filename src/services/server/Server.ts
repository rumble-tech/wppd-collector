import cors from 'cors';
import express from 'express';
import http from 'http';
import path from 'path';
import { TServerConfig } from 'src/services/config/Types';
import Logger from 'src/services/logger/Logger';
import AbstractHttpController from 'src/services/server/AbstractController';
import RouteError from 'src/services/server/RouteError';

export default class Server {
    private static instance?: Server;
    private static config: TServerConfig;

    private readonly app: express.Application;
    private readonly server: http.Server;
    private readonly router: express.Router;
    private logger: Logger;

    private constructor(logger: Logger) {
        this.app = express();
        this.server = http.createServer(this.app);
        this.router = express.Router();
        this.logger = logger;

        this.app.use(express.json());
        this.app.use(cors(Server.config.corsOptions));
        this.app.set('view engine', 'ejs');
        this.app.use(express.static(path.join(process.cwd(), 'public')));
        this.app.set('views', path.join(__dirname, '..', '..', 'views'));

        this.app.use(this.router);

        this.useErrorHandlers();
    }

    public static getInstance(logger: Logger): Server {
        if (!Server.instance) {
            Server.instance = new Server(logger);
        }

        return Server.instance;
    }

    public static unsetInstance(): void {
        Server.instance = undefined;
    }

    public static setConfig(config: TServerConfig): void {
        Server.config = config;
    }

    public async start(): Promise<void> {
        return new Promise((resolve, reject) => {
            this.server.listen(Server.config.port, () => {
                resolve();
            });

            this.server.on('error', (err) => {
                reject(err);
            });
        });
    }

    public registerController(controller: AbstractHttpController): void {
        this.router.use(controller.getPrefix(), controller.getRouter());
    }

    public getApp(): express.Application {
        return this.app;
    }

    private useErrorHandlers(): void {
        this.app.use((req: express.Request, _res: express.Response, next: express.NextFunction) => {
            const error = new RouteError(404, `Route not found: ${req.method} ${req.originalUrl}`);
            next(error);
        });

        this.app.use(
            (err: Error | RouteError, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
                const statusCode = err instanceof RouteError ? err.getStatusCode() : 500;
                const message = err.message;

                this.logger.error(`[${statusCode}] ${message}`);

                res.status(statusCode).json({
                    message,
                    data: null,
                });
            }
        );
    }
}
